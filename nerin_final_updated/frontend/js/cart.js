  ...
  confirmBtn.onclick = async () => {
    try {
      ...
      if (email) {
        ...
        const guestEmail = prompt('Ingresa tu correo electrónico para recibir el seguimiento de tu pedido:');
        if (!guestEmail) {
          alert('Debes ingresar un correo válido para continuar con la compra.');
          return;
        }
      ...
      if (res.ok) {
        ...
        alert('Pedido enviado correctamente.\nTu número de pedido es: ' + (data.orderId || 'N/A'));
        ...
        if (data.orderId) {
          ...
          try {
            await fetch(`/api/invoices/${encodeURIComponent(data.orderId)}`, { method: 'POST' });
