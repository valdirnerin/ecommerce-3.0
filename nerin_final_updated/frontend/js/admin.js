  ...
  try {
    ...
    data.products.forEach((product) => {
      ...
      tr.querySelector('.edit-btn').addEventListener('click', async () => {
        ...
        const weight = prompt('Peso (g)', product.weight != null ? product.weight : '');
        if (weight === null) return;
        const dimensions = prompt('Dimensiones', product.dimensions || '');
        if (dimensions === null) return;
        const color = prompt('Color', product.color || '');
        if (color === null) return;
        const vipChoice = confirm('¿Marcar como exclusivo VIP?\nAceptar = Sí, Cancelar = No');
...
addProductForm.addEventListener('submit', async (e) => {
  ...
  const newProd = {
    ...
    dimensions: document.getElementById('newDimensions').value.trim() || undefined,
  ...
  try {
    ...
    function buildBarChart(title, dataObj, valueSuffix = '') {
      ...
      Object.entries(dataObj).forEach(([label, val]) => {
        ...
        spanVal.textContent = valueSuffix === 'u.' ? `${val} ${valueSuffix}` : `$${val.toLocaleString('es-AR')}${valueSuffix}`;
    ...
    container.appendChild(buildBarChart('Ventas por categoría', salesByCategory, ''));
    // Unidades vendidas por producto (cantidades)
    container.appendChild(buildBarChart('Unidades vendidas por producto', salesByProduct, ' u.'));
    // Devoluciones por producto (cantidades)
    container.appendChild(buildBarChart('Devoluciones por producto', returnsByProduct, ' u.'));
    ...
    container.appendChild(buildBarChart('Clientes con mayor facturación', clientTotals, ''));
  } catch (err) {
    console.error(err);
    const container = document.getElementById('analyticsContent');
...
async function loadMetrics() {
  try {
    ...
    const totalAnnual = Object.values(m.salesByMonth).reduce((sum, t) => sum + t, 0);
