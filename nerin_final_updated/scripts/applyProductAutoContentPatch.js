...
patch("backend/services/catalogCsvImport.js", (text) => {
  ...
  if (!next.includes("quality: imported.quality,")) {
    next = next.replace(
      /(\s+csvMaximumQuantityInOrder: imported\.maximumQuantityInOrder,\n)/,
      "$1      quality: imported.quality,\n      remarks: imported.remarks,\n",
    );
    next = next.replace(
      /(\s+supplierPartNumber: imported\.supplierPartNumber,\n)/,
      "$1    quality: imported.quality,\n    remarks: imported.remarks,\n",
    );
    next = next.replace(
      /(\s+brand: imported\.manufacturerName,\n)/,
      "$1    quality: imported.quality,\n    remarks: imported.remarks,\n",
    );
  }
  return next;
...
patch("backend/server.js", (text) => {
  ...
  if (!next.includes("function isProductIndexable(product)")) {
    next = next.replace(
      ...
      `$1function isProductIndexable(product) {
...
function getProductLastModifiedDate`,
  ...
  next = next.replace(
    /function buildFeaturePhrase\(product\) \{\n(?!\s+const autoContent = buildProductAutoContent)/,
    `function buildFeaturePhrase(product) {
  const autoContent = buildProductAutoContent(product || {});
...
`,
  ...
  next = next.replace(
    /const productEntries = products\.filter\(\(p\)=>isProductPublic\(p\)\)/g,
    "const productEntries = products.filter((p)=>isProductIndexable(p))",
  );
  next = next.replace(
    /\.filter\(\(product\) => isProductPublic\(product\)\)/g,
    ".filter((product) => isProductIndexable(product))",
  );
  next = next.replace(
    /function renderProductInfoSsr\(product, siteBase\) \{\n\s+const heading = buildProductHeading\(product\);\n\s+const description = buildFeaturePhrase\(product\);/,
    `function renderProductInfoSsr(product, siteBase) {
  const autoContent = buildProductAutoContent(product || {});
  ...
  next = next.replace(
    /(\s+const stockValue = typeof product\?\.stock === "number" \? product\.stock : null;\n)\s+const technology = inferDisplayTechnology\(product\);\n\s+const frame = inferHasFrame\(product\);/,
    `$1  const specs = autoContent.technicalSpecs || {};
...
`,
  ...
  next = next.replace(
    /const compatibility = \[brand, model, sku\]\.filter\(Boolean\)\.join\(" "\);\n\s+const compatibilityHtml = compatibility\n\s+\? `<p class=\\"product-compatibility\\">Compatible con \$\{esc\(compatibility\)\}\.<\/p>`/,
    `const compatibility = autoContent.compatibilityNotice || [brand, model, sku].filter(Boolean).join(" ");
  const compatibilityHtml = compatibility
    ? \`<p class=\\"product-compatibility\\">\${esc(compatibility)}</p>\``,
  );
  next = next.replace(
    /(\s+const name = product\.name \|\| "";\n)(?!\s+const autoContent = buildProductAutoContent)/,
    `$1    const autoContent = buildProductAutoContent(product);
...
`,
  ...
  next = next.replace(
    ...
    "const seoTitle = autoContent.seoTitle || productSeo.title || buildProductSeoTitle(product);",
  );
  next = next.replace(
    /const description = productSeo\.description \|\| buildProductMetaDescription\(product\);/,
    "const description = autoContent.seoDescription || productSeo.description || buildProductMetaDescription(product);",
  );
  next = next.replace(
    /const h1 = buildProductHeading\(product\);/,
    "const h1 = autoContent.h1 || buildProductHeading(product);",
  );
  if (!next.includes("const schemaCondition = /reacondicionado/i.test")) {
    next = next.replace(
      /(\s+const formattedPrice = Number\.isFinite\(numericPrice\)\n\s+\? numericPrice\.toFixed\(2\)\n\s+: "0\.00";\n)/,
