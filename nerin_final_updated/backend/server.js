    ...
    req.on('end', () => {
      try {
        ...
        if (!user) {
          ...
          user = regUsers.find((u) => u.email === email && u.password === password);
        }
        if (user) {
      ...
      try {
        ...
        try {
          ...
          for (const item of cart) {
            const prod = products.find((p) => p.id === item.id);
            if (!prod) {
              return sendJson(res, 400, { error: `Producto con ID ${item.id} no encontrado` });
            }
            const available = typeof prod.stock === 'number' ? prod.stock : 0;
            if (item.quantity > available) {
              return sendJson(res, 400, { error: `Stock insuficiente para ${prod.name}. Disponibles: ${available}` });
            }
          }
        } catch (e) {
