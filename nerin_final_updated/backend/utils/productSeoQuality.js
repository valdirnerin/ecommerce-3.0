...
function evaluateProductSeoQuality(product = {}, autoContent = null) {
  ...
  const autoText = normalizeText(
    [content.shortDescription, content.longDescription, content.compatibilityNotice].filter(Boolean).join(" "),
  );
  ...
  const price = firstPositiveNumber([
    pickField(product, ["price_minorista", "precio_minorista", "price", "precio_final", "price_mayorista"]),
  ]);
  ...
  const signals = {
    ...
    hasSlug,
    hasIdentity,
    hasRichAutomaticDescription,
    hasPoorSupplierDescription,
    autoDescriptionLength: autoText.length,
    supplierDescriptionLength: supplierText.length,
    availability,
    availableOnRequest,
  };
  ...
  if (!qualityKnown && hasPoorSupplierDescription) {
    return {
      ...
      reason: "unknown_quality_and_poor_description",
      signals,
    };
  }
  ...
  if (qualityKnown && hasPrice && hasSlug && hasIdentity && hasRichAutomaticDescription) {
    return {
      ...
      noindex: false,
      robots: "index,follow",
      reason: "indexable_product_content",
      signals,
    };
  }
  ...
  return {
    ...
    noindex: true,
    robots: "noindex,follow",
    reason: `missing_${missing.join("_") || "seo_signals"}`,
    signals,
  };
}

module.exports = {
  evaluateProductSeoQuality,
};
